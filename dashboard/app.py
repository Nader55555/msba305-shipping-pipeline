...
DARK = dict(
    ...
    legend=dict(bgcolor="rgba(0,0,0,0)", bordercolor="#1e3a5f", font=dict(color="#ffffff")),
    margin=dict(t=40, b=40, l=50, r=20),
)

def layout(**overrides):
    """Merge DARK theme with per-chart overrides — avoids duplicate key errors."""
    d = dict(DARK)
    d.update(overrides)
    return d
C = dict(
...
def load_bigquery(project: str, dataset: str, key: str) -> dict:
    ...
    # Try Streamlit secrets first (for deployed cloud app)
    # Falls back to local key file (for local development)
    try:
        sa_info = dict(st.secrets.get("gcp_service_account", {}))
        if not sa_info: raise KeyError("no secrets")  # Streamlit Cloud
        creds   = service_account.Credentials.from_service_account_info(sa_info)
        use_project = sa_info.get("project_id", project)
    except Exception:
...
with st.sidebar:
    ...
    # Auto-detect if running on Streamlit Cloud with secrets configured
    try:
        _has_cloud_secrets = "gcp_service_account" in st.secrets
    except Exception:
        _has_cloud_secrets = False

    if _has_cloud_secrets:
        # Cloud deployment — auto-connect, no manual input needed
        use_bq     = True
        bq_project = st.secrets["gcp_service_account"].get("project_id", "msba305-shipping")
        bq_dataset = "shipping_data"
        key_file   = ""
        st.success("✅ Connected to BigQuery", icon="🔗")
    else:
        # Local development — manual sidebar input
...
with st.spinner("Loading data..."):
    try:
        ...
        _has_cloud_secrets = False
    _can_connect = use_bq and bq_project and bq_dataset and (_has_cloud_secrets or (key_file and os.path.exists(key_file)))
    if _can_connect:
        try:
...
if page == "📊 Executive Summary":
    ...
    if not bdi.empty and "bdi_value" in bdi.columns:
        ...
        fig.update_layout(**layout(height=200, hovermode="x unified",
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if "disruption_score" in df.columns and "strait_name" in df.columns:
        ...
        fig.update_layout(**layout(height=380, xaxis=dict(range=[0,105], **DARK["xaxis"]),
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if "strait_disruption_score" in rd.columns and "route" in rd.columns:
        ...
        fig.update_layout(**layout(height=350, xaxis_tickangle=-20,
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if not sig_df.empty and "market_signal" in sig_df.columns:
        ...
        fig_s.update_layout(**layout(height=220, hovermode="x unified",
                            title="BDI coloured by market signal",title_font=dict(color="white"),legend_font=dict(color="white"),
    ...
    with c1:
        ...
        if "is_spike" in df.columns:
            ...
            fig2.update_layout(**layout(barmode="group", height=280, margin=dict(t=10,b=40,l=40,r=10)))
            st.plotly_chart(fig2, use_container_width=True)
    with c2:
        ...
        if not sf.empty and "avg_bdi" in sf.columns:
            ...
            fig3.update_layout(**layout(height=280, margin=dict(t=10,b=40,l=40,r=10)))
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if not bt.empty and "avg_bdi" in bt.columns and "export_value_trillion" in bt.columns:
        ...
        bt = bt.drop_duplicates(subset=["year"]).sort_values("year")
        ...
        fig4.update_layout(**layout(height=320, hovermode="x unified",
                           margin=dict(t=10,b=40,l=40,r=60)))
        fig4.update_yaxes(title_text="USD Trillion", secondary_y=False,
                          gridcolor="#1e3a5f", color="#ffffff", title_font=dict(color="#ffffff"))
        fig4.update_yaxes(title_text="BDI",         secondary_y=True,
                          color="#ffffff", title_font=dict(color="#ffffff"))
        st.plotly_chart(fig4, use_container_width=True)
        if "correlation_signal" in bt.columns:
            with st.expander("📊 Signal details — year by year"):
    ...
    if "yoy_growth_pct" in dft.columns and "flow_direction" in dft.columns:
        ...
        fig5.update_layout(**layout(height=420, margin=dict(t=10,b=40,l=140,r=20)))
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with c1:
        ...
        if "port_name" in ws_df.columns and "wind_speed_ms" in ws_df.columns:
            ...
            fig.update_layout(**layout(height=460, xaxis_title="m/s",
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with fc3:
        locs    = ["All"] + sorted(ais["port_guess"].dropna().unique().tolist()) if "port_guess" in ais.columns else ["All"]
    ...
    if sel_loc != "All" and "port_guess"       in df_a.columns: df_a = df_a[df_a["port_guess"]==sel_loc]
    ...
    with c1:
        h("Vessel positions — global (8 straits + ports)")
        ...
        if not pos.empty:
            ...
            for cat in (pos["vessel_category"].dropna().unique() if "vessel_category" in pos.columns else ["Unknown"]):
                ...
                if not st_.empty:
                    fig.add_trace(go.Scattermapbox(
                        ...
                        marker=dict(size=5, color=CAT_COLOR.get(cat, C["gray"]), opacity=0.4),
                    ))
            fig.update_layout(
            ...
            st.plotly_chart(fig, use_container_width=True)
            st.markdown('<p style="color:#ffffff;font-size:13px;">🔴 Tankers &nbsp; 🔵 Cargo &nbsp; 🟡 Tug/Support &nbsp; 🟢 Fishing &nbsp; 🟣 Passenger &nbsp;·&nbsp; Larger dot = moving vessel</p>', unsafe_allow_html=True)
    with c2:
        if "vessel_category" in df_a.columns:
            ...
            if not vc.empty:
                ...
                fig2.update_layout(**layout(height=220, showlegend=False, margin=dict(t=10,b=10,l=10,r=10)))
                st.plotly_chart(fig2, use_container_width=True)
        if "speed_category" in df_a.columns:
            ...
            fig3.update_layout(**layout(height=220, showlegend=False,
                               margin=dict(t=10,b=50,l=30,r=10), xaxis_tickangle=-30))
            st.plotly_chart(fig3, use_container_width=True)

    # ── Top destinations ──────────────────────────────────────────────────
    if "destination" in df_a.columns:
        h("Top vessel destinations")
        dests = (df_a[df_a["destination"].notna() &
                      (df_a["destination"].str.strip() != "") &
                      (~df_a["destination"].str.startswith("@"))]
                 ["destination"].str.strip().str.upper()
                 .value_counts().head(15).reset_index())
        dests.columns = ["destination", "count"]
        if not dests.empty:
            st.caption("Where are vessels heading? Shows the 15 most common declared destinations.")
            fig_d = go.Figure(go.Bar(
                x=dests["count"], y=dests["destination"],
                orientation="h",
                marker_color=C["blue"],
                text=dests["count"],
            ...
            fig_d.update_layout(**layout(height=360,
                                         xaxis_title="Vessel count",
    ...
    if "sog_knots" in df_a.columns:
        h("Speed distribution of moving vessels")
        mv_spd = df_a[(df_a["sog_knots"].notna()) & (df_a["sog_knots"] > 0.5)]["sog_knots"]
        if not mv_spd.empty:
            st.caption("Speed distribution (knots) — shows whether vessels are transiting normally or moving slowly (possible congestion or maneuvering).")
            fig_spd = go.Figure(go.Histogram(
                x=mv_spd, nbinsx=30,
                marker_color=C["teal"], opacity=0.8,
            ))
            fig_spd.add_vline(x=10, line_dash="dash", line_color=C["amber"],
                               annotation_text="Cruising threshold",
                               annotation_font_color=C["amber"])
            fig_spd.update_layout(**layout(height=250,
...
# ══════════════════════════════════════════════════════════════════════════════
        ...
        if sf.empty:
            ...
            **Business question:** When is freight cheapest? When should you book cargo early vs
            ...
            if "avg_bdi" in sf.columns and "month_name" in sf.columns:
                ...
                fig.update_layout(**layout(height=320, xaxis_title="Month", yaxis_title="BDI",
    ...
    if not svt.empty and "strait_name" in svt.columns:
        ...
        for col in ["total_vessels","tanker_count","cargo_count","avg_speed_knots"]:
            if col in svt.columns:
                svt[col] = pd.to_numeric(svt[col], errors="coerce")

        # Filter to key straits only for cleaner chart
        key_straits = ["Strait of Hormuz","Strait of Malacca","Suez Canal",
                       "Bab el-Mandeb","Bosphorus Strait","Strait of Gibraltar"]
        svt_key = svt[svt["strait_name"].isin(key_straits)].copy()

        if not svt_key.empty and "fetch_date" in svt_key.columns:
            svt_key["fetch_date"] = pd.to_datetime(svt_key["fetch_date"], errors="coerce")

            fig_svt = go.Figure()
            strait_colors = {
                "Strait of Hormuz":    C["coral"],
                "Strait of Malacca":   C["blue"],
                "Suez Canal":          C["amber"],
                "Bab el-Mandeb":       C["red"],
                "Bosphorus Strait":    C["purple"],
                "Strait of Gibraltar": C["teal"],
            }
            for strait in key_straits:
                sub = svt_key[svt_key["strait_name"] == strait].sort_values("fetch_date")
                if not sub.empty:
                    fig_svt.add_trace(go.Scatter(
                        x=sub["fetch_date"], y=sub["total_vessels"],
                        name=strait, mode="lines+markers",
                        line=dict(color=strait_colors.get(strait, C["gray"]), width=2),
                        marker=dict(size=5),
                        hovertemplate=(
                            f"<b>{strait}</b><br>"
                            "Date: %{x}<br>"
            ...
            fig_svt.update_layout(**layout(height=320, hovermode="x unified",
                                           xaxis_title="Date",
                                           yaxis_title="Unique vessels observed"))
            st.plotly_chart(fig_svt, use_container_width=True)

            # Tanker vs cargo breakdown — latest day
            latest_svt = svt_key.sort_values("fetch_date").groupby("strait_name").last().reset_index()
            if "tanker_count" in latest_svt.columns and "cargo_count" in latest_svt.columns:
                h("Latest snapshot — tankers vs cargo at each strait")
                fig_tc = go.Figure()
                fig_tc.add_bar(x=latest_svt["strait_name"], y=latest_svt["tanker_count"],
                               name="Tankers", marker_color=C["coral"])
                fig_tc.add_bar(x=latest_svt["strait_name"], y=latest_svt["cargo_count"],
                               name="Cargo", marker_color=C["blue"])
                fig_tc.update_layout(**layout(barmode="group", height=280,
                                              xaxis_tickangle=-15,
                                              yaxis_title="Vessel count"))
                st.plotly_chart(fig_tc, use_container_width=True)
        else:
            alert_box("blue", "Strait trend data not yet available — runs daily after first AIS collection.")
    else:
        alert_box("blue", "Strait vessel trend: data will appear after daily AIS pipeline runs.")
