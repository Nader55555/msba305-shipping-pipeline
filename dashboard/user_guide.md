...
| **Ports at risk** | How many of the 20 monitored ports have dangerous wind conditions (Beaufort ≥ 7) |
...
Each card shows the strait score, affected commodities, and alternative routing options.
...
**"Trade: N/A"** means the port's trade data hasn't matched up in the analytical table — the weather data is still real and accurate.
...
### 🛥 7. Vessel Activity
**Where are ships right now?**

This page shows data from AISStream — live vessel position data collected every day from a 4-minute window.

**Key metrics:**
- **Unique vessels**: Number of distinct ships seen today
- **Moving vessels**: Ships currently underway (speed > 0.5 knots)
- **Avg speed**: Average speed across all vessels
- **Tankers**: Number of oil/chemical tankers specifically

**Congestion alerts**: Ports where the vessel count is unusually high → CONGESTION warning. Rotterdam having 2,700+ vessels is the busiest port in Europe — that's normal. Los Angeles and New York at 100+ is also normal but worth watching.

**Vessel positions map**: All captured vessels on a world map. Click on a vessel to see its name, speed, navigation status, and destination.

**Vessel types pie chart**: Breakdown by ship category.
...
**Strait vessel traffic trends** (below the map): Shows daily vessel counts at each strait over time. A downward trend at a high-risk strait often means ships are avoiding it — this is one of the earliest warning signals available.
...
|--------|-----------------|
| BDI | Monthly (manual download from investing.com) |
| Port weather | Daily automatic (GitHub Actions, 20:00 UTC) |
| Strait conditions | Daily automatic (GitHub Actions, 20:00 UTC) |
| Vessel positions | Daily automatic (GitHub Actions, 4-minute collection) |
| Trade flows (Comtrade) | Annual (manual, from UN Comtrade) |
...
*Maritime Intel Dashboard · MSBA 305 · Data sources: UN Comtrade, BDI (investing.com), OpenWeatherMap, AISStream · Storage: Google BigQuery*
